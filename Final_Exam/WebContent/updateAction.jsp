...
if(!userID.equals(debate.getUserID())){
	...
	if (request.getParameter("debateTitle") == null || request.getParameter("debateContent")==null
			|| request.getParameter("debateTitle").equals("") || request.getParameter("debateContent").equals("")){
		PrintWriter script = response.getWriter();
		script.println("<script>");
		...
		int result = debateDAO.update(debateID, request.getParameter("debateTitle") , request.getParameter("debateContent"));
		if(result == -1){
			PrintWriter script = response.getWriter();
			script.println("<script>");
